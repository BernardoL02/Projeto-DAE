...
const emit = defineEmits(['verAlertas', 'tracking']);
...
async function refresh() {
  try {
    ...
    successMessage.value = "Erro ao atualizar a tabela!";
    setTimeout(() => (successMessage.value = ''), 3000);
  }
}
...
<template>
  ...
  <div v-bind="$attrs" v-if="successMessage"
    class="fixed -top-2 ml-4 left-0 w-full flex justify-center mt-4 z-50 transition-transform transform-gpu"
