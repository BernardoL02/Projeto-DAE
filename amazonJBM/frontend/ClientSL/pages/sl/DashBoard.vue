...
const filteredProdutos = computed(() => {
    ...
    return produtos.value.filter(produto =>
        ...
        produto.categoria.toLowerCase().includes(searchProduto.value.toLowerCase())
    );
});
...
const criarEmbalagem = async () => {
    try {
        ...
        console.log('Body da requisição formatado:', JSON.stringify(requestBody, null, 2));

        // Envio da requisição
        const token = getToken();
        const response = await fetch(`${api}/embalagem`, {
            method: 'POST',
            headers: {
                Accept: 'application/json',
                'Content-Type': 'application/json',
                ...
                <div class="bg-white w-1/2 p-6 rounded shadow-lg relative">
                    ...
                    <form @submit.prevent="criarTipo" class="space-y-4">
                        <div>
                            ...
                            <input id="tipo" v-model="novoTipo.tipo" type="text"
                                class="w-full border border-gray-300 p-2 rounded"
                ...
                <div class="bg-white w-1/2 p-6 rounded shadow-lg relative">
                    ...
                    <form @submit.prevent="criarEmbalagem" class="space-y-4">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative">
                                <ul v-if="showSensorSuggestions"
                                    class="absolute z-10 w-full bg-white border border-gray-300 rounded max-h-48 overflow-y-auto shadow-lg">
                                    <li v-for="sensor in filteredTiposSensores" :key="sensor.id"
                                        @click="addSensor(sensor)" class="p-2 hover:bg-gray-100 cursor-pointer">
                                        {{ sensor.tipo }}
                                    </li>
                                </ul>
                            </div>
                        ...
                        <div v-if="selectedSensores.length > 0" class="mb-4">
                            <h3 class="font-medium text-gray-700 mb-2">Sensores Selecionados:</h3>
                            <ul class="space-y-2">
