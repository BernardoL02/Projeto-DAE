<template>
  <div class="table-container p-8">
    <div :class="{'overflow-y-auto max-h-96': tableData.length > 9}" class="shadow-lg rounded-lg relative">
        ...
        <tbody>
          <tr v-for="(row, rowIndex) in tableData" :key="rowIndex" class="odd:bg-gray-100 even:bg-white hover:bg-indigo-50 transition duration-300">
            ...
            <td class="py-4 px-6 text-center border border-gray-300 relative">
              <input v-model="row.valor" :disabled="row.estado === 'inativo'" class="border rounded px-2 py-1 text-center w-full" />
            ...
            <td class="py-4 px-6 text-center border border-gray-300">
              <button v-if="row.estado !== 'inativo'" @click="emit('update', row)" class="bg-green-500 text-white py-1 px-3 rounded hover:bg-green-700 transition mr-2">Confirmar</button>
              <button v-if="row.estado !== 'inativo'" @click="emit('cancel', row)" class="bg-red-500 text-white py-1 px-3 rounded hover:bg-red-700 transition">Cancelar</button>
            </td>
          </tr>
        </tbody>
      </table>
